...
function contains(haystack, needle) {
	...
	while (length, length--) {
		if (!haystack.inArray(needle[length])) {
			return false;
		}
	}
	return true;
 ...
 */
 ...
 module.exports = function(targetApi, keys, callbackUrl) {
  ...
  var retrieveAccessTokens = resteasy.retrieveAccessTokens = function(request, callback) {
  ...
  var request = resteasy.request = function(method, path, params, callback) {
	...
	console.log(map.hosts.rest + path + paramAppender + querystring.stringify(params));
    // TODO: add support for all 4 CRUD actions
  ...
  var scan = resteasy.scan = function(action, section, params) {
    ...
    while (length, length--) {
	  var haystack = map[action][section][length].required || [];
      if (contains(haystack, Object.keys(params))) {
        return params, map[action][section][length];
      }
    }
	console.log('Map endpoint not found');
  };
  ...
  var create = resteasy.create = function(section, params, callback) {
    ...
    request('POST', match.path, params, callback);
  };
  ...
  var read = resteasy.read = function(section, params, callback) {
    ...
    request('GET', match.path, params, callback);
  };
  ...
  var update = resteasy.update = function(section, params, callback) {
    ...
    request('PUT', match.path, params, callback);
  };
  ...
  var del = resteasy.del = function(section, params, callback) {
    ...
    request('DELETE', match.path, params, callback);
  };
